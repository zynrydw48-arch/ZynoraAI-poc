# Zynora AI v1.0.0 — Final Release Document

Prepared 2026-07-20. This is the authoritative release record for Zynora AI
...
- **Version + About**: Help > About Zynora AI shows the current version and
...
- **Background health**: `ResourceMonitor` + `PowerStateProvider`
  ...
  attribute CPU usage correctly (excluding Zynora AI's own indexing load) to
...
| Frozen build size (`dist/ZynoraAI/`) | ~4.5 GB (bundles Python runtime, Tesseract, and all model weights for fully offline operation) |
| Installer size | ~3.23 GB (3,231,113,249 bytes) |

## 9. Installer checksum

`packaging/installer_output/ZynoraAI-Setup-1.0.0.exe`:
